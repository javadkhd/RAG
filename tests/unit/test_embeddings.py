...
class TestOllamaEmbeddingProvider:
    ...
    async def test_embed_single_text(self):
        ...
        with patch("app.providers.embeddings.ollama.httpx.AsyncClient", return_value=mock_client):
...
class TestBgeEmbeddingProvider:
    ...
    async def test_embed(self):
        ...
        with patch.object(sys.modules["sentence_transformers"], "SentenceTransformer", return_value=mock_model):
            provider = BgeEmbeddingProvider(model_name="BAAI/bge-m3", device="cpu", normalize=True)
            result = await provider.embed(["hello", "world"])
...
class TestE5EmbeddingProvider:
    ...
    async def test_embed(self):
        ...
        with patch.object(sys.modules["sentence_transformers"], "SentenceTransformer", return_value=mock_model):
            provider = E5EmbeddingProvider(model_name="intfloat/e5-large-v2", device="cpu", normalize=False)
            result = await provider.embed(["test"])

        assert result == [[0.5, 0.6]]
        mock_model.encode.assert_called_once_with(["test"], normalize_embeddings=False)
...
class TestNomicEmbeddingProvider:
    ...
    async def test_embed(self):
        ...
        with patch.object(sys.modules["sentence_transformers"], "SentenceTransformer", return_value=mock_model):
            provider = NomicEmbeddingProvider(model_name="nomic-ai/nomic-embed-text-v1.5", device="cpu", normalize=True)
            result = await provider.embed(["query"])

        assert result == [[0.7, 0.8]]
        mock_model.encode.assert_called_once_with(["query"], normalize_embeddings=True)
